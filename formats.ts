...
},
{
	...
	onModifyDamage(damage, source, target, move) {
		// Override Parental Bond damage for PHNN - second hit deals 50% instead of 25%
		if (move.multihitType === 'parentalbond' && move.hit > 1) {
			// The base game already applied 0.25, so we need to multiply by 2 to get to 0.5
		...
		actions: {
			modifyDamage(baseDamage, pokemon, target, move, suppressMessages) {
				...
				if (move.spreadHit) {
					...
					// Parental Bond modifier - PHNN uses 0.5 (50% damage) instead of 0.25
